...
public abstract class GenerateTemplate {
    ...
    public  void doGenerate() throws TemplateException, IOException, InterruptedException{
        ...
        // 1.复制原始文件
        String sourceCopyDestPath = copySource(meteObject, outputPath);

        // 2.代码生成
        generateCode(meteObject, outputPath);

        // 3.构建 jar 包
        String jarPath = buildJar(outputPath, meteObject);

        // 4.封装脚本
        String shellOutputPath = buildScript(outputPath, jarPath);

        // 5.生成精简版的程序（产物包）
        buildeDist(outputPath, shellOutputPath, jarPath,sourceCopyDestPath);
    }
    ...
    protected  String buildJar(String outputPath,Meta meteObject) throws IOException, InterruptedException {
    ...
    protected  void generateCode(Meta meteObject, String outputPath) throws IOException, TemplateException {
        // 读取 resources 目录
    ...
    protected  String copySource(Meta meteObject, String outputPath) {
        // 从原始模板文件路径复制到生成的代码中
